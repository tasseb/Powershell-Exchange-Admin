<#
.Synopsis
   Cette fonction permet d'ajouter des délégations sur des boîtes email Exchange à l'aide d'un fichier CSV passé en paramètre, avec le droit FullControl uniquement.
.DESCRIPTION
   Cette fonction permet d'ajouter des délégations sur des boîtes email Exchange avec un accès complet à la boîte.
...
function Add-UserDelegationCSV
{
    ...
    Process
    {
        ...
        Import-CSV $Path -Header MailboxName,UserName -Delimiter ";" | Foreach-Object{
            ...
            Write-Host""
            Add-UserDelegation -Target $_.MailboxName -User $_.UserName        
